## 👋 Olá! Eu sou o João Victor

Atuo na interseção entre **operações, sistemas e dados**, com experiência prática em ambientes corporativos de grande porte. Atualmente, venho aprofundando meus estudos em **desenvolvimento backend**, utilizando **Java (POO e Spring Boot)** e **Python**, criando projetos com regras de negócio próximas do mundo real, APIs REST e automações voltadas à área operacional e financeira.

### 🛠️ Tecnologias & Ferramentas
- **Java** (POO, Collections, Streams, Spring Boot)
- **Python** (automações, análise de dados, validação de arquivos fiscais)
- **SQL** (MySQL, PostgreSQL)
- **APIs REST**
- **Excel Avançado & Power BI**
- **Git & GitHub**

### 📌 O que você vai encontrar aqui
- Projetos backend com foco em **boas práticas e arquitetura**
- Simulações de sistemas reais (estoque, cartão de crédito, validações fiscais)
- Projetos voltados à **eficiência operacional e melhoria de processos**
- Estudos contínuos em **engenharia de software e dados**

📫 **Vamos nos conectar**  
- LinkedIn: *[perfil](https://www.linkedin.com/in/joao-victor-macedo-neves/)*

# EN

👋 Hi! I'm João Victor

I work at the intersection of operations, systems, and data, with hands-on experience in large corporate environments. Currently, I am deepening my studies in backend development, working with Java (OOP and Spring Boot) and Python, building projects that simulate real-world business rules, REST APIs, and automations applied to operational and financial contexts.

🛠️ Technologies & Tools

Java (OOP, Collections, Streams, Spring Boot)
Python (automation, data analysis, fiscal file validation)
SQL (MySQL, PostgreSQL)
REST APIs
Advanced Excel & Power BI
Git & GitHub

📌 What you’ll find here

Backend projects focused on clean code and good architecture
Real-world system simulations (inventory, credit card, fiscal validation)
Projects aimed at operational efficiency and process improvement
Continuous learning in software engineering and data

📫 Let’s connect

LinkedIn: *[profile](https://www.linkedin.com/in/joao-victor-macedo-neves/)*
